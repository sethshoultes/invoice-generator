...
-- This file initializes the PostgreSQL database for the Invoice Generator
...
-- For now, allow all operations (will add user-specific policies later)
CREATE POLICY "Allow all for authenticated users" ON public.clients
    FOR ALL USING (true);

CREATE POLICY "Allow all for authenticated users" ON public.invoices
    FOR ALL USING (true);

CREATE POLICY "Allow all for authenticated users" ON public.line_items
    FOR ALL USING (true);

CREATE POLICY "Allow all for authenticated users" ON public.recurring_items
    FOR ALL USING (true);

CREATE POLICY "Allow all for authenticated users" ON public.source_statements
    FOR ALL USING (true);
...
INSERT INTO public.clients (name, company, email, address_line1, city, state, zip)
VALUES (
    'Test Client',
    'Test Company Inc.',
    'test@example.com',
